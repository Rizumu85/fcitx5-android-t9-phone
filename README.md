...
可以在设置首页【中文输入方案】里勾选需要保留的方案。默认只启用拼音九键；启用多个方案后，在没有正在输入的编码时短按 **#键** 会按“拼音 -> 五笔画 -> 注音”的顺序快速切换。完整的 Rime 方案仍可从快捷设置【词库切换】进入。
...
中文模式通用规则：
- 输入编码时短按 **#键** 会提交上方预览的拼音、笔画或注音本身，不会提交汉字候选；空闲且只保留一个中文方案时，短按 **#键** 仍执行回车/搜索。
- 长按 **#键** 切换中文、英文、数字顶层模式，并清除尚未提交的中文编码。
- 短按 **\*键** 打开中文符号候选；如果正在输入，会先提交当前汉字候选再打开符号。长按 **\*键** 输入字面量 `*`。
- 输入编码时长按数字键可按候选下方的小数字快捷选择第 1-10 个汉字候选；空闲时长按数字键输入数字。
- 方向键移动候选，确认键提交当前候选，上下键翻页。拼音九键还可以在上方拼音筛选栏里选择读音。
- 拼音九键输入中短按 **1键** 可分隔拼音，例如 `ka'le`；提交拼音预览时不会带上空格和分词符号。
- 注音九键暂不占用数字键输入声调，候选会根据无声调注音和使用记录排序。

### 英文模式

普通英文九键使用 multi-tap 输入。需要预测英文单词时，可以在输入法上方【⋯】快捷设置里切换到【联想英语】。

联想英语使用英文 T9 词库：按数字序列后，候选区显示完整预测词，上方输入行显示当前实际输入进度。确认键可以提交当前候选，左右键可移动候选，长按数字键可快捷选择对应候选。
...
- 长按 `#` 切换中文/英文/数字模式
- 中文模式->可在设置中保留需要的拼音、五笔画、注音方案，空闲时短按 `#` 快速切换
