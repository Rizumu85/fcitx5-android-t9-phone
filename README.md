...
- 有拼音候选时短按 **0键** 可以确认当前候选，不会额外插入空格；长按 **0键** 仍然选择第 10 个汉字候选。
- 点按屏幕上的回车键可以直接提交当前拼音预览（不带空格和分词符号）。
- 汉字候选和符号候选下方的小数字是物理键快捷选择提示。

### 英文模式

普通英文九键使用 multi-tap 输入。需要预测英文单词时，可以在输入法上方【⋯】快捷设置里切换到【智能英语】。

智能英语使用英文 T9 词库：按数字序列后，候选区显示完整预测词，上方输入行显示当前实际输入进度。确认键可以提交当前候选，左右键可移动候选，长按数字键可快捷选择对应候选。

智能英语学习词库的使用方法：
- 先在快捷设置里关闭【智能英语】，回到普通英文九键。
- 用普通英文九键打出一个连续英文单词，再按空格、标点或回车结束这个词；例如打完 `rizum` 后按空格，它会被记录到本地学习词库。
- 再切回【智能英语】，输入这个词对应的 T9 数字序列，它就可以作为候选出现。
- 只会学习 2 个字母以上的纯英文字母词；密码输入框、声明不允许个性化学习的输入框不会记录。
- 如果误学了验证码、错词或不想保留的词，可以到设置首页【词库管理】->【智能英语学习词库】里添加、编辑或删除。
...
- 智能英语模式（英文 T9 预测、词库候选、普通英文九键学习新词、学习词库管理）
...
- 英文模式->支持大小写
- 智能英语模式->支持英文 T9 预测、更完整的英文词库、普通英文九键学习新词和学习词库管理
