...
可以在设置首页【中文输入方案】里勾选需要保留的方案。默认只启用拼音九键；启用多个方案后，在没有正在输入的编码时长按 **\*键** 会按“拼音 -> 五笔画 -> 注音”的顺序快速切换。完整的 Rime 方案仍可从快捷设置【词库切换】进入。
...
中文模式通用规则：
- 输入编码时短按 **#键** 会提交上方预览的拼音、笔画或注音本身，不会提交汉字候选；空闲时短按 **#键** 始终执行回车/搜索。
- 长按 **#键** 切换中文、英文、数字顶层模式，并清除尚未提交的中文编码。
- 短按 **\*键** 打开中文符号候选；如果正在输入，会先提交当前汉字候选再打开符号。输入编码时长按 **\*键** 输入字面量 `*`；空闲时长按 **\*键** 切换已启用的中文方案，只有一个方案时输入字面量 `*`。
