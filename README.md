...
可以在设置首页【中文输入方案】里勾选需要保留的方案，并分别设置拼音、笔画、注音默认输出简体或繁体。默认只启用拼音九键；启用多个方案后，长按 **\*键** 会按“拼音 -> 笔画 -> 注音”的顺序快速切换。完整的 Rime 方案仍可从快捷设置【词库切换】进入。

默认简繁会在进入方案时应用一次，不会锁定当前会话；仍可用 Rime 快捷选项临时切换，离开并重新进入该方案时恢复设置的默认值。
...
中文模式规则：
- 短按 * 打开符号输入， 再短按 * 切换中英符号；长按 * 切换中文的拼音->笔画->注音模式。
...
- 长按 `#` 切换中文/英文/数字模式
- 中文模式->可在设置中保留需要的拼音、笔画、注音方案，分别设置默认简繁输出，空闲时长按 `*` 快速切换
