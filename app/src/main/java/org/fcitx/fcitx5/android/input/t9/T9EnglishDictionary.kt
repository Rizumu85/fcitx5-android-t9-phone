...
class T9EnglishDictionary {
    ...
    @Volatile
    private var builtInIndex = T9EnglishBuiltInIndex.Empty
    @Volatile
    ...
    fun preload() {
        ...
        val loaded = T9ResponsivenessTrace.measure("T9EnglishDictionary.preload.parse") {
            loadBuiltInIndex()
        }
        T9ResponsivenessTrace.measure("T9EnglishDictionary.preload.warmPrefixes") {
            loaded.warmPrefixes(WarmupDigitSequences)
        }
        synchronized(this) {
            if (builtInReady) return
            builtInIndex = loaded
    ...
    fun candidatesFor(digits: String, limit: Int = 10): List<String> =
        ...
        if (builtInReady) {
            builtInIndex.exactWords(digits)
                .forEach { if (addCandidate(it)) return cacheCandidates(cacheKey, candidates) }
        }
        ...
        if (builtInReady) {
            builtInIndex.prefixWords(digits)
                .forEach { if (addCandidate(it)) return cacheCandidates(cacheKey, candidates) }
        }
    ...
    private fun loadBuiltInIndex(): T9EnglishBuiltInIndex =
        runCatching {
            appContext.assets.open(BuiltInDictionaryAsset).bufferedReader().useLines { lines ->
                T9EnglishBuiltInIndex.parse(lines)
            }
        }.getOrDefault(T9EnglishBuiltInIndex.Empty)
