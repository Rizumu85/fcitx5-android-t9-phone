...
安装或替换 APK 后，不要沿用安装前一直开着的文本框。先关闭并重新打开
测试 App，再重新点进文本框，等输入法方案名和候选界面加载完成后才开始
注入物理按键。旧文本框可能保留失效的 `InputConnection`，表现为候选仍会
变化、`commitText()` 也返回成功，但文字不会进入 App；这不是输入规则本身
的回归。
