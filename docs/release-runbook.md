...
| `release-baidu/vX.Y.Z/` | Local staging folder for the version uploaded to Baidu | The four distributed APKs and matching readme text files; add the Rime configuration archive only when that release changes it | Debug APKs, unsigned APKs, x86/x86_64 APKs unless explicitly requested |
...
Only when the app release changes `RIME_CONFIG_BASELINE_VERSION`, also download
that release's `rime-ice-t9-phone-main.zip` into the staging folder and verify
it against the digest declared in `app/build.gradle.kts`. Do not repeatedly
attach an unchanged configuration archive to later Baidu releases.

Before asking the user to upload, verify the staging folder contains exactly
the four release APKs and two readme files, plus the Rime archive only when the
...
- [ ] Local Baidu staging contains exactly four APKs and Markdown/text readmes; it contains the matching Rime configuration archive only when this release changes that configuration.
